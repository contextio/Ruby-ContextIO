module ContextIO
  class Message < BaseClass
    MESSAGE_READERS = %I(date date_indexed date_received addresses person_info email_message_id
                       message_id gmail_message_id gmail_thread_id files subject
                       folders sources content type charset body_section answered
                       deleted draft flagged seen in_reply_to references resource_url)
    private
    attr_reader :parent

    public
    attr_reader :status, :success, :connection, :message_id, *MESSAGE_READERS
    def initialize(parent:,
                   identifier: nil,
                   response: nil,
                   status: nil,
                   success: nil)
      @parent = parent
      @connection = parent.connection
      @message_id = identifier
      @status = status
      @success = success
      if response
        parse_response(response)
      end
    end

    def call_url
      build_url("messages", message_id)
    end

    def body
      call_instance_endpoint("#{call_url}/body")
    end

    def flags
      call_instance_endpoint("#{call_url}/flags")
    end

    def folders
      call_instance_endpoint("#{call_url}/folders")
    end

    def headers
      call_instance_endpoint("#{call_url}/headers")
    end

    def source
      call_instance_endpoint("#{call_url}/source")
    end

    def threads
      call_instance_endpoint("#{call_url}/thread")
    end
  end
end
